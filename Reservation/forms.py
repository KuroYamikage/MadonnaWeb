...
from Reservation.reservation_function.availability import (
    check_availability2,
    check_availability,
    check_availability3,
)
...
class ReservationForm(forms.ModelForm):
    model = Reservations
    discountAmount = forms.CharField(
    ...
    class Meta:
        ...
        widgets = {
            ...
            "facility": CustomCheckboxSelectMultiple(
                attrs={
                    "class": "form-control form-row d-flex justify-content-center align-items-center h-20  ",
                    "onclick": "facilitiesFee(" ")",
                    "id": "id_facility",
                }
            ),
            "discount": forms.TextInput(attrs={"class": "form-control"}),
            "timeIn": forms.TimeInput(
                attrs={"class": "form-control", "type": "time", "readonly": "True"}
            ),
            "timeOut": forms.TimeInput(
                attrs={"class": "form-control", "type": "time", "readonly": "True"}
            ),
            "referenceNum": forms.TextInput(
...
class ReservationEditForm(forms.ModelForm):
    ...
    class Meta:
        ...
        model = Reservations
        discountAmount = forms.IntegerField(
        ...
        widgets = {
            ...
            "status": forms.RadioSelect(
                ...
                attrs={
                    "class": "d-flex form-control justify-content-center align-items-center h-20  "
                },
            ),
        }
    ...
    def __init__(self, *args, **kwargs):
        ...
        super(ReservationEditForm, self).__init__(
            *args, **kwargs
    ...
    def clean(self):
        ...
        for price in prices_list:
            print(check_availability(price, checkIn, checkOut))
            if check_availability3(
                price, checkIn, checkOut, timeIn, timeOut, self.user
            ):
                available_price.append(price)
        if len(available_price) > 0:
            av_price = available_price[0]
...
class PriceForm(forms.ModelForm):
    ...
    class Meta:
        model = Prices
        dayTimeChoices = (("day", "Day"), ("night", "Night"), ("whole", "Whole Day"))
        ...
        widgets = {
            ...
            "maxPax": forms.RadioSelect(
                choices=maxPaxChoices, attrs={"class": "form-control"}
            ),
            "dayTime": forms.RadioSelect(
                choices=dayTimeChoices, attrs={"class": "form-control"}
            ),
        }

        label = {"price": "Price", "dayTime": "Schedule", "maxPax": "Maximum Guest"}
...
class ReservationChecker(forms.Form):
    ...
    def clean(self):
        ...
        for price in prices_list:
            print(
                "test"
                + str(check_availability2(price, checkIn, checkOut, timeIn, timeOut))
            )
            if check_availability2(price, checkIn, checkOut, timeIn, timeOut):
                available_price.append(price)
        if len(available_price) > 0:
